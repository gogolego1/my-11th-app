...
없는 항목은 제외하고 있는 항목만 포함하세요.""",
    ...
    for col, (etype, (label, key)) in zip(upload_cols, exam_labels.items()):
        with col:
            f = st.file_uploader(label, type=ACCEPT, key=key,
                                 help="이미지(PNG/JPG/WEBP) 또는 PDF")
            uploaded[etype] = f
            if f:
                if f.name.lower().endswith(".pdf"):
                    st.success(f"📄 PDF 업로드됨\n{f.name}")
                else:
                    f.seek(0)
    ...
    if any_uploaded:
        ...
        if do_parse and has_api:
            parsed = {}
            prog = st.progress(0)
            total = sum(1 for v in uploaded.values() if v)
            done = 0
            for etype, f in uploaded.items():
                ...
                f.seek(0)
                file_bytes = f.read()
                with st.spinner(f"{exam_labels[etype][0]} 파싱 중... ({f.name})"):
                ...
                if result:
                    ...
                    st.warning(f"⚠ {exam_labels[etype][0]} 파싱 실패 — 파일 형식·내용을 확인하세요")
            prog.empty()
    ...
    if p:
        ...
        verify_tabs = st.tabs([exam_labels[k][0] for k in p.keys()])
