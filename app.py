...
없는 항목은 제외하고 있는 항목만 포함하세요.""",
        ...
        # Gemini 2.5 Flash Preview (2025년 최신 모델)
        url = (
    ...
    uploaded_files = st.file_uploader(
        "검사 결과지 파일 선택 (여러 개 동시 가능)",
        type=ACCEPT,
        accept_multiple_files=True,
        help="이미지(PNG/JPG/WEBP) 또는 PDF — 최대 4개(검사별 1개)"
    )

    # 업로드된 파일 미리보기
    if uploaded_files:
        prev_cols = st.columns(min(len(uploaded_files), 4))
        for col, f in zip(prev_cols, uploaded_files):
            with col:
                if f.name.lower().endswith(".pdf"):
                    st.info(f"📄 {f.name}")
                else:
                    f.seek(0)
                    st.image(f.read(), use_column_width=True, caption=f.name)
                    f.seek(0)

        # 각 파일이 어느 검사인지 사용자가 지정
        st.markdown('<div class="section-title">📋 파일별 검사 유형 지정</div>',
                    unsafe_allow_html=True)
        st.caption("Gemini가 파일 내용을 보고 파싱합니다. 파일이 어떤 검사인지 확인하세요.")

        file_types = {}
        type_cols = st.columns(min(len(uploaded_files), 4))
        etype_options = list(exam_labels.values())  # 표시용
        etype_keys    = list(exam_labels.keys())     # 내부 키

        for col, f in zip(type_cols, uploaded_files):
            with col:
                chosen = st.selectbox(
                ...
                file_types[f.name] = etype_keys[etype_options.index(chosen)]

    # ── 파싱 실행 버튼 ──────────────────────────────
    any_uploaded = bool(uploaded_files)
    if any_uploaded:
        ...
        with btn_col2:
            if not has_api:
                st.caption("API Key가 없으면 파싱이 실행되지 않습니다.")
        ...
        if do_parse and has_api:
            ...
            for f in uploaded_files:
                etype = file_types.get(f.name, "vark")
                mime = get_mime(f.name)
                f.seek(0)
                file_bytes = f.read()
                with st.spinner(f"{exam_labels[etype]} 파싱 중... ({f.name})"):
                ...
                if result:
                    parsed[etype] = result
                    st.success(f"✅ {exam_labels[etype]} 파싱 완료")
                else:
                    st.warning(f"⚠ {exam_labels[etype]} 파싱 실패 — 파일 형식·내용을 확인하세요")
            prog.empty()
