...
if page == "📥 데이터 입력":
    ...
    if not api_key:
        api_key = st.sidebar.text_input(
            ...
            placeholder="AIza...",
            help="파일 자동 파싱에 필요합니다. Streamlit secrets에 GEMINI_API_KEY로 등록하면 자동 로드됩니다."
        )

    # ── 파일 → Gemini 파싱 함수 ─────────────────────
    def parse_file_with_gemini(file_bytes: bytes, mime_type: str,
                               exam_type: str, api_key: str) -> dict:
        """Gemini 2.0 Flash API로 검사 결과지(이미지/PDF)를 파싱."""
        import base64, urllib.request, urllib.error

        PROMPTS = {
            "vark": """이 파일은 학습유형검사(VARK) 결과지입니다.
반드시 아래 JSON만 출력하세요. 설명, 주석, 마크다운 없이 JSON만 출력하세요.
{"vark":"시각형","scores":{"시각형":0,"청각형":0,"읽기쓰기형":0,"운동감각형":0}}

[읽기 방법]
1순위: 텍스트로 명시된 유형명과 점수를 읽으세요.
   예) "시각형: 12점" / "V=12, A=8, R=10, K=5"
2순위: 텍스트 수치가 없으면 그래프/차트 막대 높이를 Y축 최대값 대비 비율로 환산하세요.
   예) Y축 최대 20, 시각형 막대 높이 약 12 → 점수 12

[영역명 매핑]
- V / Visual / 시각적 / 시각형 → "시각형"
- A / Aural / Auditory / 청각적 / 청각형 → "청각형"
- R / Read / Reading / 읽기·쓰기 / 읽기쓰기형 → "읽기쓰기형"
- K / Kinesthetic / 체험 / 운동감각형 → "운동감각형"

[규칙]
- vark 값은 4가지 중 가장 높은 점수 유형
- scores 숫자는 따옴표 없이 숫자만
- 점수가 전혀 없으면 scores를 {}로""",

            "holland": """이 파일은 진로탐색검사(Holland RIASEC) 결과지입니다.
반드시 아래 JSON만 출력하세요. 설명, 주석, 마크다운 없이 JSON만 출력하세요.
{"holland":"탐구형(I)","scores":{"R":0,"I":0,"A":0,"S":0,"E":0,"C":0}}

[읽기 방법]
1순위: 텍스트로 명시된 유형명과 점수를 읽으세요.
   예) "탐구형(I): 15점" / "R=8, I=15, A=10, S=12, E=9, C=7"
2순위: 텍스트 수치가 없으면 그래프/차트 막대를 Y축 최대값 대비 비율로 환산하세요.
3순위: "1순위 유형: 탐구형" / "최고 유형" 등 텍스트 표현도 활용하세요.

[유형 코드 매핑]
- R / 현실형 / Realistic → "R"
- I / 탐구형 / Investigative → "I"
- A / 예술형 / Artistic → "A"
- S / 사회형 / Social → "S"
- E / 진취형 / Enterprising → "E"
- C / 관습형 / Conventional → "C"

[holland 값 규칙]
- 가장 높은 점수 유형을 "탐구형(I)", "예술형(A)", "사회형(S)", "진취형(E)", "현실형(R)", "관습형(C)" 중 하나로
- scores 숫자는 따옴표 없이 숫자만
- 점수가 전혀 없으면 scores를 {}로""",

            "sel": """이 파일은 사회정서역량검사(SEL/CASEL) 결과지입니다.
반드시 아래 JSON만 출력하세요. 설명, 주석, 마크다운 없이 JSON만 출력하세요.
{"자기인식":"보통","자기조절":"보통","사회적 인식":"보통","관계 기술":"보통","책임 있는 의사결정":"보통"}

[읽기 우선순위]
1순위: 텍스트로 명시된 수준 표현을 읽으세요.
   예) "○○ 영역은 상 수준입니다" / "중 수준" / "하 수준"
   예) "상위권" / "중위권" / "하위권"
   예) "우수" / "보통" / "노력 필요" / "부족"
2순위: 텍스트 수준이 없으면 그래프 막대 높이를 Y축 최대값 대비 비율로 계산하세요.
   예) Y축 최대 40, 막대 높이 약 36 → 36/40 = 90% → 높음

[수준 변환 규칙]
- "상 수준" / "상위권" / "우수" / "높음" → "높음"
- "중 수준" / "중위권" / "보통" / "평균" → "보통"
- "하 수준" / "하위권" / "노력 필요" / "부족" / "낮음" → "지원 필요"
- 점수 기준: 80점 이상(또는 상위 80%) → "높음", 60~79점 → "보통", 60점 미만 → "지원 필요"
- 그래프 비율 기준: 75% 이상 → "높음", 50~74% → "보통", 50% 미만 → "지원 필요"

[영역명 매핑]
- "자기인식" / "자아인식" / "자기 인식" → "자기인식"
- "자기조절" / "자기 조절" / "자기관리" → "자기조절"
- "사회적 인식" / "사회인식" / "공감" → "사회적 인식"
- "관계 기술" / "긍정적 관계 맺기" / "관계기술" / "대인관계" → "관계 기술"
- "책임 있는 의사결정" / "책임감" / "의사결정" → "책임 있는 의사결정"

[중요] 키 이름은 반드시 "자기인식", "자기조절", "사회적 인식", "관계 기술", "책임 있는 의사결정" 5개 중 해당하는 것만 포함하세요. 이미지에 일부 영역만 있으면 그것만 포함하세요.""",

            "diag": """이 파일은 기초학력진단검사 결과지입니다.
반드시 아래 JSON만 출력하세요. 설명, 주석, 마크다운 없이 JSON만 출력하세요.
{"수학":{"수와 연산":75.0,"도형":80.0},"과학":{"물질":65.0}}

[읽기 방법]
1순위: 표/테이블에 명시된 정답률(%), 점수, 성취도를 읽으세요.
   예) "수와 연산: 75.0%" / "정답률: 80.0"
2순위: 텍스트 수치가 없으면 그래프 막대를 Y축 최대값(보통 100) 대비 비율로 환산하세요.
3순위: O/X 정오표가 있으면 (맞은 문항 수 / 전체 문항 수) × 100으로 계산하세요.

[규칙]
- 있는 교과와 영역만 포함, 없는 것은 제외
- 정답률은 따옴표 없이 숫자(소수점 1자리)
- 교과명: "수학", "국어", "과학" 중 해당하는 것만
- 영역명은 결과지에 나온 그대로 (예: "수와 연산", "도형", "물질" 등)""",
        }

        b64 = base64.standard_b64encode(file_bytes).decode()

        # Gemini 2.5 Flash (안정 버전 — preview 접미사 없음)
        url = (
        ...
        def extract_json(text: str) -> dict:
            ...
            # 주석 제거 // ...
            text = re.sub(r"//[^\n]*", "", text)
            # 숫자 값의 따옴표 제거  "score": "82" → "score": 82
        ...
        try:
            ...
            st.error(f"Gemini API 오류 ({e.code}): {err_body[:400]}")
            return {}
        except json.JSONDecodeError as e:
            st.error(f"JSON 파싱 오류: {e}")
            st.code(raw[:500], language="text")   # 원본 응답 표시
            return {}
    ...
    stu_num = st.number_input("학생 번호", min_value=1, max_value=40,
                              value=1, key="stu_num_input")

    # ── 파일 업로드 섹션 ────────────────────────────
    st.markdown('<div class="section-title">📂 검사 결과지 파일 업로드</div>',
                unsafe_allow_html=True)
    st.caption("이미지(PNG·JPG·WEBP) 또는 PDF 파일을 올리면 Gemini가 자동으로 읽어드립니다.")

    has_api = bool(api_key)
    if not has_api:
        st.warning("⚠ 사이드바에 **Gemini API Key**를 입력해야 자동 파싱이 실행됩니다.")
    ...
    # ── 검사별 탭 + 다중 파일 업로드 ────────────────
    st.caption("각 검사 탭에서 파일을 여러 개 올릴 수 있습니다. 이미지(PNG·JPG·WEBP)와 PDF 모두 지원합니다.")

    upload_tabs = st.tabs(list(exam_labels.values()))
    uploaded = {}   # {etype: [file, ...]}

    for tab, (etype, label) in zip(upload_tabs, exam_labels.items()):
        with tab:
            files = st.file_uploader(
                f"{label} 결과지 파일 (여러 개 가능)",
                type=ACCEPT,
                accept_multiple_files=True,
                key=f"upload_{etype}",
                help="같은 검사 결과지를 여러 장(이미지 분할 캡처 등) 올릴 수 있습니다."
            )
            uploaded[etype] = files or []

            # 미리보기
            if files:
                prev_cols = st.columns(min(len(files), 4))
                for col, f in zip(prev_cols, files):
                    with col:
                        if f.name.lower().endswith(".pdf"):
                            st.info(f"📄 {f.name}", icon="📄")
                        else:
                            f.seek(0)
                            st.image(f.read(), use_column_width=True,
                                     caption=f.name[:25])
                            f.seek(0)
                color = exam_colors[etype]
                st.markdown(
                    f'<div style="font-size:12px;color:{color};margin-top:4px">'
                    f'✅ {len(files)}개 파일 업로드됨</div>',
                    unsafe_allow_html=True)

    # ── 파싱 실행 버튼 ──────────────────────────────
    any_uploaded = any(len(v) > 0 for v in uploaded.values())
    if any_uploaded:
        # 업로드 현황 요약
        summary_parts = [
            f"{exam_labels[et]}({len(fs)}개)"
            for et, fs in uploaded.items() if fs
        ]
        st.caption(f"📂 업로드 현황: {' · '.join(summary_parts)}")

        btn_col1, btn_col2 = st.columns([2, 3])
        with btn_col1:
            do_parse = st.button(
                "🔍 자동 파싱 실행", type="primary",
                disabled=not has_api,
                use_container_width=True,
                help="Gemini API Key가 필요합니다" if not has_api else ""
            )
        with btn_col2:
            if not has_api:
                st.caption("사이드바에 Gemini API Key를 입력하면 활성화됩니다.")

        if do_parse and has_api:
            parsed = {}
            total = sum(len(fs) for fs in uploaded.values() if fs)
            prog = st.progress(0)
            done = 0

            for etype, files in uploaded.items():
                if not files:
                    continue

                # 파일이 여러 개면 → 각각 파싱 후 결과 병합
                merged: dict = {}
                all_ok = True
                ...
                for f in files:
                    ...
                    file_bytes = f.read()
                    with st.spinner(
                        f"{exam_labels[etype]} 파싱 중… ({f.name})"
                    ):
                        result = parse_file_with_gemini(
                            file_bytes, mime, etype, api_key)
                    done += 1
                    prog.progress(done / total)

                    if result:
                        # 기초학력진단은 교과별로 딕셔너리 병합
                        if etype == "diag":
                            for subj, areas in result.items():
                                if subj not in merged:
                                    merged[subj] = {}
                                merged[subj].update(areas)
                        elif etype == "sel":
                            # SEL 영역명 정규화 (검사지마다 표현이 다름)
                            SEL_KEY_MAP = {
                                "자기인식": "자기인식", "자아인식": "자기인식",
                                "자기 인식": "자기인식",
                                "자기조절": "자기조절", "자기 조절": "자기조절",
                                "자기관리": "자기조절",
                                "사회적 인식": "사회적 인식", "사회인식": "사회적 인식",
                                "공감": "사회적 인식", "사회적인식": "사회적 인식",
                                "관계 기술": "관계 기술", "긍정적 관계 맺기": "관계 기술",
                                "관계기술": "관계 기술", "대인관계": "관계 기술",
                                "관계": "관계 기술",
                                "책임 있는 의사결정": "책임 있는 의사결정",
                                "책임감": "책임 있는 의사결정",
                                "의사결정": "책임 있는 의사결정",
                                "책임있는의사결정": "책임 있는 의사결정",
                            }
                            normalized = {
                                SEL_KEY_MAP.get(k, k): v
                ...
                if merged:
                    parsed[etype] = merged
                    status = "완료" if all_ok else "일부 파싱 실패 (나머지 결과 저장)"
                    st.success(f"✅ {exam_labels[etype]} {status}")
                else:
                    st.warning(
                        f"⚠ {exam_labels[etype]} 파싱 실패 — "
                        "파일 형식·내용을 확인하세요")
    ...
    if p:
        st.markdown('<div class="section-title">📋 파싱 결과 확인</div>',
                    unsafe_allow_html=True)
        st.caption("Gemini가 읽어낸 데이터입니다. 이상한 값이 있으면 파일을 다시 올려주세요.")

        verify_tabs = st.tabs([exam_labels[k] for k in p.keys()])
        for tab, (etype, data) in zip(verify_tabs, p.items()):
            with tab:
                vcol1, vcol2 = st.columns([1, 1])
                with vcol1:
                    st.markdown("**추출된 JSON**")
                    st.json(data)
                with vcol2:
                    st.markdown("**해석 결과**")
                    if etype == "vark":
                        ...
                        st.metric("학습유형", val)
                        if data.get("scores"):
                            for t, sc in data["scores"].items():
                                pct = min(int(sc) if isinstance(sc, (int, float)) else 0, 100)
                                st.markdown(
                                    f'<div style="display:flex;align-items:center;gap:6px;margin:3px 0">'
                                    f'<span style="font-size:12px;min-width:76px">{t}</span>'
                                    f'<div style="flex:1;background:#EEE;border-radius:4px;height:8px">'
                                    f'<div style="width:{pct}%;background:#1565C0;height:8px;border-radius:4px"></div></div>'
                                    f'<span style="font-size:12px;min-width:28px;text-align:right">{sc}</span></div>',
                                    unsafe_allow_html=True)
                    elif etype == "holland":
                        val = data.get("holland", "")
                        st.metric("진로유형", val)
                        if data.get("scores"):
                            sc_map = {"R":"현실형","I":"탐구형","A":"예술형",
                                      "S":"사회형","E":"진취형","C":"관습형"}
                            scores = data["scores"]
                            max_sc = max(scores.values()) if scores else 1
                            for code, sv in scores.items():
                                pct = int(sv / max_sc * 100) if max_sc else 0
                                st.markdown(
                                    f'<div style="display:flex;align-items:center;gap:6px;margin:3px 0">'
                                    f'<span style="font-size:12px;min-width:76px">{sc_map.get(code,code)}</span>'
                                    f'<div style="flex:1;background:#EEE;border-radius:4px;height:8px">'
                                    f'<div style="width:{pct}%;background:#2E7D32;height:8px;border-radius:4px"></div></div>'
                                    f'<span style="font-size:12px;min-width:28px;text-align:right">{sv}</span></div>',
                                    unsafe_allow_html=True)
                    elif etype == "sel":
                        lv_color = {"높음":"#2E7D32","보통":"#E65100","지원 필요":"#C62828"}
                        for domain, lv in data.items():
                            color = lv_color.get(lv, "#888")
                            st.markdown(
                                f'<div style="display:flex;justify-content:space-between;'
                                f'padding:5px 8px;margin:3px 0;border-radius:6px;'
                                f'background:{color}15;border-left:3px solid {color}">'
                                f'<span style="font-size:12px">{domain}</span>'
                                f'<b style="font-size:12px;color:{color}">{lv}</b></div>',
                                unsafe_allow_html=True)
                    elif etype == "diag":
                        for subj, areas in data.items():
                            st.markdown(f"**{subj}**")
                            for area, score in areas.items():
                                color = "#C62828" if float(score) < 70 else "#2E7D32"
                                warn = " ⚠" if float(score) < 70 else ""
                                st.markdown(
                                    f'<div style="display:flex;align-items:center;gap:6px;margin:2px 0">'
                                    f'<span style="font-size:11.5px;min-width:100px">{area}{warn}</span>'
                                    f'<div style="flex:1;background:#EEE;border-radius:4px;height:7px">'
                                    f'<div style="width:{min(float(score),100)}%;background:{color};height:7px;border-radius:4px"></div></div>'
                                    f'<span style="font-size:11.5px;color:{color};min-width:36px;text-align:right">{score}%</span></div>',
                                    unsafe_allow_html=True)
        ...
        with save_col1:
            parsed_list = [exam_labels[k][0] for k in p]
            st.success(f"✅ 파싱 완료: {', '.join(parsed_list)} — 저장 버튼을 눌러 확정하세요.")
        with save_col2:
            if st.button("💾 데이터 저장", type="primary", use_container_width=True):
                # 파싱 결과에서 값 추출
                vark_val = p.get("vark", {}).get("vark", "시각형")
                if vark_val not in VARK_TYPES:
                    vark_val = "시각형"

                holland_val = p.get("holland", {}).get("holland", "탐구형(I)")
                if holland_val not in HOLLAND_TYPES:
                    holland_val = "탐구형(I)"

                sel_val = {}
                sel_raw = p.get("sel", {})
                for domain in SEL_DOMAINS:
                    lv = sel_raw.get(domain, "보통")
                    sel_val[domain] = lv if lv in ["높음","보통","지원 필요"] else "보통"
                ...
                for subj, areas in SUBJECT_AREAS.items():
                    diag_val[subj] = {}
                    for area in areas:
                        diag_val[subj][area] = float(
                            diag_raw.get(subj, {}).get(area, 75.0))
                ...
                st.session_state.parsed = {}
                st.success(
                    f"✅ {stu_num}번 학생 저장 완료! "
                    f"(총 {len(st.session_state.students)}명 등록)"
                )
    else:
        # 파싱 결과 없을 때 안내
        if not any_uploaded:
            st.info("위에서 검사 결과지 파일을 업로드한 후 '자동 파싱 실행'을 클릭하세요.")

    # ── 저장된 학생 목록 ────────────────────────────
    if st.session_state.students:
        st.markdown("---")
        st.markdown('<div class="section-title">저장된 학생 목록</div>',
                    unsafe_allow_html=True)
        summary = []
...
# ───────────────────────────────────────────────
    ...
    st.title("🔬 검사지 심층 분석")
    st.caption("검사 결과지 이미지를 올리면 텍스트를 수합·분석하여 ⓐ개인 특성 ⓑ학급 특성 ⓒ수업설계 제안을 도출합니다.")
    ...
    if not api_key:
        api_key = st.sidebar.text_input(
            "Gemini API Key", type="password",
            placeholder="AIza...", key="analysis_api_key"
        )
    has_api = bool(api_key)

    # ── Gemini 호출 함수 ─────────────────────────
    def call_gemini_text(prompt: str, api_key: str,
                         images: list = None) -> str:
        """텍스트(+선택적 이미지들)로 Gemini 호출 → 텍스트 응답 반환."""
        import base64, urllib.request, urllib.error
        ...
        try:
            ...
            return f"[API 오류 {e.code}]: {e.read().decode()[:300]}"
        except Exception as e:
            return f"[오류]: {e}"
    ...
    def get_mime(filename):
        ...
        return {"pdf":"application/pdf","png":"image/png",
                "jpg":"image/jpeg","jpeg":"image/jpeg","webp":"image/webp"}.get(ext,"image/png")
    ...
    # ════════════════════════════════════════════
    if mode == "👤 학생 개인 분석":
        st.markdown('<div class="section-title">검사 결과지 이미지 업로드</div>',
                    unsafe_allow_html=True)
        st.caption("한 학생의 검사 결과지를 모두 올려주세요. 여러 검사, 여러 장 가능합니다.")

        uploaded = st.file_uploader(
            "검사 결과지 파일 (여러 개 동시 가능)",
            type=ACCEPT,
            accept_multiple_files=True,
            key="analysis_individual"
        )

        if uploaded:
            # 미리보기
            prev_cols = st.columns(min(len(uploaded), 4))
            for col, f in zip(prev_cols, uploaded):
                with col:
                    if f.name.lower().endswith(".pdf"):
                        st.info(f"📄 {f.name}")
                    else:
                        f.seek(0)
                        st.image(f.read(), use_column_width=True, caption=f.name[:20])
                        f.seek(0)

        st.markdown("---")

        if st.button("🔬 개인 심층 분석 실행", type="primary",
                     disabled=not has_api or not uploaded,
            ...
            for f in uploaded:
                f.seek(0)
                images.append((f.read(), get_mime(f.name)))

            PROMPT_INDIVIDUAL = """당신은 초등학교 교사를 돕는 학습자 분석 전문가입니다.

첨부된 이미지들은 한 학생의 검사 결과지입니다 (학습유형검사·진로탐색검사·사회정서역량검사·기초학력진단 중 일부).

[분석 지시]
이미지에서 텍스트, 수치, 그래프, 수준 표현을 모두 읽어서 아래 3가지를 분석하세요.
그래프가 있으면 막대 높이를 Y축 최대값 대비 비율로 수치화하세요.
수준 표현(상/중/하, 상위권/중위권/하위권 등)도 반드시 반영하세요.
...
## ⓒ 이 학생을 위한 수업설계 제안
- HEAL 단계별 추천 역할 (H·E·A·L 각각)
- 핵심 질문 개인화 방향
- 피드백 방식 추천
- 모둠 구성 시 고려사항

---
[출력 형식] 마크다운으로 작성하세요. 각 항목은 구체적이고 실천 가능하게 서술하세요."""

            with st.spinner("이미지 텍스트 수합 및 분석 중... (30~60초 소요)"):
                result = call_gemini_text(PROMPT_INDIVIDUAL, api_key, images)

            st.session_state["analysis_individual_result"] = result

        # 결과 표시
        if "analysis_individual_result" in st.session_state:
            result = st.session_state["analysis_individual_result"]

            if result.startswith("[API 오류") or result.startswith("[오류]"):
                st.error(result)
            else:
                # 3개 탭으로 분리 표시
                ta, tb, tc = st.tabs([
                ...
                with ta:
                    section = extract_section(result, "ⓐ 학생 개인 특성 분석")
                    st.markdown(section if section else result)

                with tb:
                    section = extract_section(result, "ⓑ 이 학생을 위한 핵심 교육적 시사점")
                    st.markdown(section if section else "")

                with tc:
                    section = extract_section(result, "ⓒ 이 학생을 위한 수업설계 제안")
                    st.markdown(section if section else "")
    ...
    # ════════════════════════════════════════════
        ...
        st.markdown('<div class="section-title">학급 전체 검사 결과지 업로드</div>',
                    unsafe_allow_html=True)
        st.caption("여러 학생의 결과지를 모두 올려주세요. 이미지·PDF 혼합 가능합니다.")
        ...
        for tab, (label, ekey) in zip(class_tabs, exam_key_map.items()):
            with tab:
                files = st.file_uploader(
                    f"{label} 결과지 (여러 학생 파일 동시 가능)",
                    type=ACCEPT,
                    accept_multiple_files=True,
                    key=f"class_{ekey}"
                )
                class_uploaded[ekey] = files or []
                if files:
                    st.caption(f"✅ {len(files)}개 파일 업로드됨")
                    prev_cols = st.columns(min(len(files), 5))
                    for col, f in zip(prev_cols, files):
                        with col:
                            if f.name.lower().endswith(".pdf"):
                                st.info(f"📄 {f.name[:15]}")
        ...
        total_files = sum(len(v) for v in class_uploaded.values())
        st.markdown("---")

        if total_files > 0:
            st.caption(f"📂 총 {total_files}개 파일 업로드됨")

        if st.button("🔬 학급 전체 심층 분석 실행", type="primary",
                     disabled=not has_api or total_files == 0,
                     use_container_width=False):

            # 검사 유형별로 이미지 수집
            all_images_by_type = {}
            for ekey, files in class_uploaded.items():
                imgs = []
                for f in files:
                    f.seek(0)
                    imgs.append((f.read(), get_mime(f.name)))
                if imgs:
                    all_images_by_type[ekey] = imgs
            ...
            prog = st.progress(0)
            step = 0
            total_steps = len(all_images_by_type) + 1  # +1: 최종 분석

            for ekey, imgs in all_images_by_type.items():
                step += 1
                prog.progress(step / total_steps)
                exam_name = exam_name_map[ekey]

                with st.spinner(f"{exam_name} 결과지 텍스트 수합 중... ({len(imgs)}개 파일)"):
                    extract_prompt = f"""이 이미지들은 학급 학생들의 {exam_name} 결과지입니다.
이미지에서 읽을 수 있는 모든 텍스트, 수치, 수준 표현, 그래프 값을 있는 그대로 수합해주세요.
그래프가 있으면 막대 높이를 Y축 최대값 대비 비율로 환산한 수치도 포함하세요.
학생 이름은 제외하고 번호나 익명으로만 처리하세요.
수합 결과를 그대로 나열해주세요. 분석은 하지 마세요."""
                    text = call_gemini_text(extract_prompt, api_key, imgs)
                    collected_texts[exam_name] = text

            # Step 2: 수합된 텍스트로 학급 분석
            step += 1
            prog.progress(step / total_steps)
            combined = "\n\n".join([
                f"=== {name} 수합 결과 ===\n{text}"
                for name, text in collected_texts.items()
            ])

            PROMPT_CLASS = f"""당신은 초등학교 교사를 돕는 학습자 분석 전문가입니다.

아래는 한 학급 학생들의 검사 결과지에서 수합한 텍스트입니다.
학생 이름은 없고 번호 또는 익명으로 처리되어 있습니다.
...
### HEAL 단계별 설계 방향
- H단계 (탐구·Hook): 이 학급에 맞는 도입 방식
- E단계 (탐구·Explore): 학습유형 다양성을 살리는 탐구 구성
- A단계 (적용·Apply): 강점 기반 역할 배분 방향
- L단계 (공유·Launch): 이 학급에 맞는 발표·공유 방식

### 모둠 구성 원칙
- 이 학급 데이터 기반 모둠 구성 시 고려사항

### 우선 지원 전략
- 즉시 적용 가능한 지원 방안 3가지

---
[출력 형식] 마크다운으로 구체적이고 실천 가능하게 서술하세요. 데이터 근거를 명시하세요."""

            with st.spinner("학급 전체 심층 분석 생성 중..."):
                final_result = call_gemini_text(PROMPT_CLASS, api_key)

            prog.empty()
            st.session_state["analysis_class_result"] = final_result
            st.session_state["analysis_class_texts"] = collected_texts

        # 결과 표시
        if "analysis_class_result" in st.session_state:
            result = st.session_state["analysis_class_result"]
            ...
            if result.startswith("[API 오류") or result.startswith("[오류]"):
                ...
                with ta:
                    s = extract_section(result, "ⓐ 학급 학생들의 인지·정의·행동 특성 분석")
                    st.markdown(s if s else result)

                with tb:
                    s = extract_section(result, "ⓑ 학급 전체 특성 종합 분석")
                    st.markdown(s if s else "")

                with tc:
                    s = extract_section(result, "ⓒ 이 학급을 위한 수업설계 제안")
                    st.markdown(s if s else "")

                # 수합된 원본 텍스트 확인
                if "analysis_class_texts" in st.session_state:
                    with st.expander("📄 수합된 원본 텍스트 확인", expanded=False):
                        for name, text in st.session_state["analysis_class_texts"].items():
                            st.markdown(f"**{name}**")
                ...
                st.download_button(
                    "📥 학급 분석 결과 다운로드 (.txt)",
                    data=result,
                    file_name="학급_전체_심층분석.txt",
                    mime="text/plain",
                    use_container_width=False
                )

    if not has_api:
        st.warning("⚠ 사이드바에 Gemini API Key를 입력해야 분석이 실행됩니다.")
